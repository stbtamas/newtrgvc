...
for(const [input,wanted]of [['/','/'],['/community/','/community/'],['/community/login','/community/'],['/community/releases/lights','/community/'],['/community/sw.js','/community/sw.js']]){const r=await worker.fetch(new Request(origin+input));assert.equal(await r.text(),wanted);assert.equal(r.headers.get('x-content-type-options'),'nosniff')}
